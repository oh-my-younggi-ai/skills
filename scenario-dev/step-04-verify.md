...
- **Scenario auditor** — gets the diff + the scenario artifact. Asks: does each test *actually assert its Then*, or is it hollow / tautological (e.g. asserts a mock it set up, or `expect(true)`)? Does the implementation genuinely satisfy each frozen scenario, or merely pass a weak test? For every scenario marked `manual`, asks whether its stated reason still holds now that the code exists — an assertion that looked impossible during capture is often reachable once the seam is actually built.
...
- **automatable_manual** — a scenario marked `manual` whose Then can in fact be asserted now. Write the test, flip the mark to `auto`, and cover it in `./step-03-cover.md`. This is the drift the manual mark is most likely to hide.
