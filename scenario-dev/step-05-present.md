...
1. Set status `done`.
2. If version control is available and the tree is dirty, create a local commit with a conventional message derived from the title. Never push.
3. Open the artifact in the human's editor so they can click through the traceability: resolve the repo root (`git rev-parse --show-toplevel`, falling back to the current working directory) and the artifact's absolute path, then run `code -r "{root}" "{artifact}"` (root first, both double-quoted). If `code` isn't available, just tell them the path.
4. **Summarize in chat** (paths CWD-relative, with `:line`):
